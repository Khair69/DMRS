...
namespace DMRS.Api.Infrastructure.Search.Medication
{
    public class MedicationRequestIndexer : ResourceSearchIndexerBase
    {
        ...
        public override List<ResourceIndex> Extract(Resource resource)
        {
            ...
            AddIndex(indices, medicationRequest.Id, "status", medicationRequest.StatusElement?.JsonValue?.ToString());
            AddIndex(indices, medicationRequest.Id, "intent", medicationRequest.IntentElement?.JsonValue?.ToString());
            AddIndex(indices, medicationRequest.Id, "priority", medicationRequest.PriorityElement?.JsonValue?.ToString());
