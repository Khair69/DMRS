...
{
    public class ProcedureIndexer : ISearchIndexer
    {
        ...
        public List<ResourceIndex> Extract(Resource resource)
        {
            ...
            foreach (var category in procedure.Category?.Coding ?? [])
                AddIndex(indices, procedure.Id, "category", category.Code);

            AddIndex(indices, procedure.Id, "category", procedure.Category?.Text);
