...
{
    public class ServiceRequestIndexer : ISearchIndexer
    {
        ...
        public List<ResourceIndex> Extract(Resource resource)
        {
            ...
            AddIndex(indices, serviceRequest.Id, "code", serviceRequest.Code?.Text);
            foreach (var coding in serviceRequest.Code?.Coding ?? [])
