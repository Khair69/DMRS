...
{
    public class AppointmentIndexer : ResourceSearchIndexerBase
    {
        protected override string ResourceType => "Appointment";
        ...
        public override List<ResourceIndex> Extract(Resource resource)
        {
            ...
            AddIndex(indices, appointment.Id, "status", appointment.StatusElement?.JsonValue?.ToString());
            ...
            {
                AddIndex(indices, appointment.Id, "service-type", serviceType.Reference.Reference);
                foreach (var coding in serviceType.Concept.Coding)
