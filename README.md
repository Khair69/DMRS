# Nabd (نبض) — Digital Medical Records System · نظام السجلات الطبية الرقمية
...
«نبض» منصة شاملة لإدارة السجلات الطبية الرقمية تم بناؤها باستخدام ASP.NET Core وBlazor، وتطبّق معايير
...
Nabd is a modern, standards-based healthcare information system designed to manage patient medical
...
> **Naming note:** *Nabd* (نبض) is the product name. Code artifacts — solution projects, namespaces,
> the PostgreSQL databases, and the Keycloak realm — keep the historical `DMRS` prefix, so paths and
> configuration below are unchanged.
